...
class SequenceLoader:
    def __init__(self, npz_path, input_length, label_length,train):
        """
        初始化加载器
        ...
        self.index = np.load("random_numbers.npz",allow_pickle=True)['random_numberslist']
        if train=="train":
            self.thisIndex = self.index[0:520000]
        elif train=="test":
            self.thisIndex = self.index[520000:585000]
        else:
            self.thisIndex = self.index[585000:]
    ...
    def __getitem__(self, index):
        ...
        index=self.thisIndex[index]
        start_idx = index
        end_idx = index + self.input_length*100 + self.label_length*100

        sequence = self.sequences[start_idx:end_idx:100]
        ...
        # print(inputseq.size())
        if inputseq.size() == torch.Size([0, 2]):
            print("Warning: inputseq size is torch.Size([0, 2])")
        return inputseq, labelseq

# 使用示例
if __name__ == "__main__":
    loader = SequenceLoader("train.npz", input_length=10, label_length=5,train="train")
